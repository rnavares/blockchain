...
def test_is_valid_block_bad_proof_of_work(last_block, block):
    ...
    with pytest.raises(Exception, match="The proof of requirement ot met"):
        Block.is_valid_block(last_block, block)


def test_is_valid_block_has_jump_difficulty(last_block, block):
    jumped_difficulty = 10
    block.difficulty = jumped_difficulty
    block.hash = f'{"0" * jumped_difficulty}11abc'

    with pytest.raises(Exception, match="Block difficulty must only adjust by one"):
        Block.is_valid_block(last_block, block)
